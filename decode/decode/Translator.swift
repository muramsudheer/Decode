...
public class Translator {
    ...
    public func translate(input: String, to toLanguage: String, completion: @escaping (TranslationResult) -> ()) {
        ...
        case .available(let token):
            Alamofire.request(API.translateURL, method: .get, parameters: ["text": input, "to": toLanguage], headers: ["Ocp-Apim-Subscription-Key": "43936858599b473babbe5ac2ecab16fb"]).responseString(completionHandler: { (response) in
                switch response.result {
                case .success(let str):
